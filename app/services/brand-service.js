...
exports.updateBrand = async (id, name) => {
  const results = await Brand.update(
    { name },
    { where: { id } })
  return results[0]
}
...
exports.deleteBrand = async (brandId) => {
  ...
  const result = db.sequelize.transaction(async (t) => {
    const deleteResult = await Brand.destroy(
      { where: { id: brandId } },
      { transaction: t })
    const updatePlacingResults = []
    for (let i = brand.placing; i < brandCount - 1; i++) {
      const updatePlacingResult = await Brand.update(
        { placing: i },
        { where: { superTH: brand.superTH, placing: i + 1 } },
        { transaction: t })
      updatePlacingResults.push(updatePlacingResult)
    }
    return [deleteResult, ...updatePlacingResults].includes(0) ? 0 : 1
  })
  return result
}
